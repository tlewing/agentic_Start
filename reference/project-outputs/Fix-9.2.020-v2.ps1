...
$item = $items | Where-Object {
    $_.FieldValues["FileLeafRef"] -like "9.2.020*Procurement*"
}
...
if ($item) {
    ...
    # List all 9.2.020 files
    Write-Host "`nAll 9.2.020 files:" -ForegroundColor Yellow
    $all9220 = $items | Where-Object { $_.FieldValues["FileLeafRef"] -like "9.2.020*" }
    foreach ($f in $all9220) {
