<#
.SYNOPSIS
    Simple fix - strip ALL dashes from title, rebuild with exactly one
#>
...
Write-Host "  Simple Fix - Remove Extra Dashes" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
foreach ($item in $items) {
    ...
    if ($baseName -match "^([\d\.]+)") {
        ...
        $rest = $baseName.Substring($Matches[0].Length)
    }
    else {
    ...
    # Clean the title - remove ALL dashes
    $title = Get-CleanTitle -text $rest
    ...
    # Build new name: "9.2.010 - Team Selection" (one en-dash)
