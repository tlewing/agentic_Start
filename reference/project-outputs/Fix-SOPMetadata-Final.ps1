<#
.SYNOPSIS
    Final cleanup - fix remaining title issues
#>
...
Write-Host "  Fix SOP Metadata - Final Cleanup" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
function Clean-Title {
    ...
    # Remove "SOP" with any surrounding separators
    $title = $title -replace "SOP\s*[-_]?\s*", ""

    # Remove leading dashes, underscores, spaces (repeat to catch multiple)
    ...
    # Fix common patterns
    $title = $title -replace "\s*-\s*-\s*", " - "  # Double dashes
    $title = $title -replace "^\s*-\s*", ""        # Leading dash with spaces
    $title = $title -replace "\(\s*", "("          # Space after open paren
    $title = $title -replace "\s*\)", ")"          # Space before close paren
...
foreach ($item in $items) {
    ...
    Write-Host "  Title: $sopTitle" -ForegroundColor Cyan
